/**
 * C48「领取条件」级联选项树（主进程副本）
 * 数据源自 freelive-browser-extension claim-condition/trees.ts（2026-08-06 CDP 实页）
 * 页面侧逻辑使用 vendor 内同名数据，此处供 payload 校验与 UI 表单渲染。
 */

export interface ClaimConditionNode {
  value: string
  label: string
...
export const C48_DEFAULT_CLAIM_CONDITION_PATH: ClaimConditionPath = ['不限']

export function normalizeClaimConditionPath(path: unknown): ClaimConditionPath {
  if (!Array.isArray(path)) return []
  return path.filter((p): p is string => typeof p === 'string').map((p) => p.trim()).filter(Boolean)
}
